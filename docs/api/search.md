...
|range       |int     |locationパラメータで指定した位置情報から検索に含める半径を指定します. 単位はメートルです. `range`パラメータは`location`パラメータと一緒に指定してください, `location`パラメータまたは`range`パラメータのどちらか一つしか指定しなかった場合は`400`を返します.|
